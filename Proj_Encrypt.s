...
ENC_GotKey:
        ...
        movf    enc_tmp, W, A
        sublw   '9'
        bnc     ENC_KeyEntryLoop    ; > '9', ignore

        movf    enc_tmp, W, A
        sublw   '0' - 1
        bnc     ENC_KeyEntryLoop    ; < '0', ignore
...
ENC_TryConfirm:
        ...
        call    ENC_ParsePair0
	movwf   enc_sub1, A     ; store result; Z flag set if value is zero
	bz      ENC_Invalid     ; fail if sub1 == 0
	movlw   27
	cpfslt  enc_sub1, A     ; skip if enc_sub1 < 27
	bra     ENC_Invalid     ; fail if enc_sub1 >= 27
...
ENC_UpdateShift:
        ...
        movf    enc_shift, W, A
        addwf   enc_run_shift, F, A ; enc_run_shift += enc_shift

        ; Wrap into range 1-36
ENC_ShiftWrapLoop:
        movlw   ALPHA_SIZE + 1      ; 37
        cpfslt  enc_run_shift, A
        bra     ENC_ShiftWrapReduce
        bra     ENC_ShiftWrapDone

ENC_ShiftWrapReduce:
        movlw   ALPHA_SIZE
        subwf   enc_run_shift, F, A
        bra     ENC_ShiftWrapLoop
...
ENC_DisplayResult:
        ...
        ; Position cursor at line 1 column 0
        movlw   LCD_LINE1_BASE
        call    LCD_Send_Byte_I
        movlw   10
        call    LCD_delay_x4us
...
ENC_DisplayLoop:
        ...
        ; Load message_buffer[enc_index] (encrypted content)
        lfsr    0, message_buffer
        movf    enc_index, W, A
        addwf   FSR0L, F, A
        movlw   0
        addwfc  FSR0H, F, A
        movf    INDF0, W, A
        movwf   enc_tmp, A          ; stash: LCD calls will clobber W

        ; Characters 0-15: print at natural cursor position
        movlw   16
        cpfslt  enc_index, A
...
ENC_ShiftAndPrint:
        ...
        ; Reposition cursor to right edge:
        ; after (enc_index - 15) left-shifts, right edge = 0x80 + enc_index
        movf    enc_index, W, A
        addlw   LCD_LINE1_BASE
        call    LCD_Send_Byte_I
        movlw   10
        call    LCD_delay_x4us
...
ENC_CharToPos:
        movf    enc_tmp, W, A
        sublw   'A' - 1             ; carry set if enc_tmp >= 'A'
        bnc     ENC_CheckDigit

        movf    enc_tmp, W, A
        sublw   'Z'                 ; carry set if enc_tmp <= 'Z'
        bnc     ENC_NotAlpha
